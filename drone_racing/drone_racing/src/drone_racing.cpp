...
DroneRacing::DroneRacing(const ros::NodeHandle &nh, const ros::NodeHandle &pnh)
    : nh_(nh), pnh_(pnh), desired_state_quad_(), desired_state_world_(),
      state_estimate_quad_(), state_estimate_world_(),
      state_machine_(State::kOff), copilot_is_in_feedthrough_(false),
      goal_selected_(false), distance_to_goal_(0.0), tf_listener_() {
    loadParameters();
    state_estimate_sub_ = nh_.subscribe(
        "state_estimate", 1, &DroneRacing::stateEstimateCallback, this);
    start_navigation_sub_ = nh_.subscribe(
        "start_navigation", 1, &DroneRacing::startNavigationCallback, this);
    hard_stop_sub_ =
        nh_.subscribe("hard_stop", 1, &DroneRacing::hardStopCallback, this);
    cnn_sub_ =
        nh_.subscribe("/cnn_out/traj", 1, &DroneRacing::networkCallback, this);
    test_time_sub_ = nh_.subscribe("only_network", 1,
                                   &DroneRacing::enableTestTimeCallback, this);

    run_idx_sub_ =
        nh_.subscribe("run_idx", 1, &DroneRacing::setRunIdxCallback, this);
    env_setup_sub_ = nh_.subscribe(
        "setup_environment", 1, &DroneRacing::setupEnvironmentCallback, this);
    ...
    desired_state_pub_ = nh_.advertise<quadrotor_msgs::TrajectoryPoint>(
    ...
    gazebo_rviz_visualizer_.reset(
        new gazebo_rviz_visualizer::GazeboRvizVisualizer);
    environment_is_set_up_ = false;
    main_loop_timer_ =
        nh_.createTimer(ros::Duration(0.02), &DroneRacing::mainloop, this);
}
...
void DroneRacing::setupEnvironmentCallback(const std_msgs::EmptyConstPtr &msg) {
    ...
    last_goal_index_ = (curr_goal_index_ + goal_positions_.size() - 1) %
                       goal_positions_.size();
...
void DroneRacing::mainloop(const ros::TimerEvent &time) {
    ...
    if (environment_is_set_up_) {
        if (moving_gates_ && (ros::WallTime::now() - start_time_).toSec() >
                                 sec_no_record_at_start_) {
        ...
        case State::kRacing: {
            desired_state_world_ =
                trajectory_manager_->getDesiredStateFromTrajectory();
        ...
        if ((ros::WallTime::now() - start_mainloop).toSec() > 0.02) {
            ROS_ERROR(
                "[%s] Mainloop iteration took [%f] sec, state_machine_ = [%d]",
                ros::this_node::getName().c_str(),
                (ros::WallTime::now() - start_mainloop).toSec(),
...
void DroneRacing::updateStateMachine() {
    if (state_machine_ == State::kOff || state_machine_ == State::kHover) {
        ...
        if (setGoalWasTriggered()) {
            global_trajectory_->findStartIndexGlobalTrajectory(
                getEstimatedPos());
            global_trajectory_->findStartIndexNWGlobalTrajectory(this);
            ROS_INFO("[%s] Entering racing mode.",
                     ros::this_node::getName().c_str());
            mainloop_iter_ = 0;
...
quadrotor_common::TrajectoryPoint DroneRacing::getEndState() {
    ...
    double dist_to_next_gate =
        (desired_state_world_.position - goal_positions_.at(curr_goal_index_))
            .norm();
    double dist_to_last_gate =
        (desired_state_world_.position - goal_positions_.at(last_goal_index_))
            .norm();

    double horizon =
        std::max(horizon_min_, std::min(dist_to_next_gate, dist_to_last_gate));
    ...
    visualizer_->displayDebug(end_state.position.x(), end_state.position.y(),
                              end_state.position.z(), 0,
...
void DroneRacing::performNavigation() {
    if (checkStateEstDesStateDivergence()) {
        ROS_WARN("[%s] Desired state and state estimate diverged.",
                 ros::this_node::getName().c_str());
        ROS_WARN("Desired state: [%f, %f, %f]",
                 desired_state_world_.position.x(),
                 desired_state_world_.position.y(),
                 desired_state_world_.position.z());
        ROS_WARN("State estimate: [%f, %f, %f]",
                 state_estimate_world_.pose.pose.position.x,
                 state_estimate_world_.pose.pose.position.y,
                 state_estimate_world_.pose.pose.position.z);
...
void DroneRacing::collectData() {
    ...
    if (mainloop_iter_ % plan_every_n_iter_ == 0) {
        ...
        double desired_velocity_mb =
            getDesiredVelocity(); // normalized with max global velocity; [0,1]
        end_state = getEndState();

        double v_rescaled = desired_velocity_mb * max_velocity_;
        double v_execute =
            std::min(desired_state_world_.velocity.norm() + 0.5, v_rescaled);
        ...
        state_estimate_pose.position.x =
            state_estimate_world_.pose.pose.position.x;
        state_estimate_pose.position.y =
            state_estimate_world_.pose.pose.position.y;
        state_estimate_pose.position.z =
            state_estimate_world_.pose.pose.position.z;
        state_estimate_pose.orientation =
        ...
        double yaw_to_end =
            std::atan2(T_B_goal.getPosition().y(), T_B_goal.getPosition().x());
        ...
        double pitch_to_end =
            std::atan2(T_B_goal.getPosition().z(), dist_to_goal);
        ...
        data_saver_->setModelBasedPrediction(Eigen::Vector3d(
            traj_info.idx_right, traj_info.idx_up, traj_info.curr_vel));
        bool success;
        if (perturb_actions_) {
            Eigen::Vector3d random_perturbation = Eigen::Vector3d(
                0.1 * std::sin(0.5 * ros::WallTime::now().toSec()),
                0.1 * std::sin(0.5 * ros::WallTime::now().toSec()),
                0.05 * std::sin(0.5 * ros::WallTime::now().toSec()));
            success = trajectory_manager_->computeTrajectory(
                this, desired_state_world_, state_estimate_world_,
                Eigen::Vector3d(traj_info.idx_right, traj_info.idx_up,
                                traj_info.curr_vel) +
                    random_perturbation);
        } else {
            success = trajectory_manager_->computeTrajectory(
        ...
        if ((ros::WallTime::now() - start_time_).toSec() >
            sec_no_record_at_start_) {
            if (record_data_) {
        ...
        if (success) {
            ...
            if (failed_trials_ < max_failed_trials_) {
                failed_trials_++;
                ROS_WARN("[%s] Failed trials: [%d].",
                         ros::this_node::getName().c_str(), failed_trials_);
            } else {
                ROS_INFO("[%s] Going to Hover state.",
                         ros::this_node::getName().c_str());
                resetDesiredState();
                publishDesiredState();
                data_saver_->stopRecording();
...
void DroneRacing::resetDesiredState() {
    ...
    desired_state_quad_.time_from_start =
        ros::Duration(0.0); // ros::Time::now();
    desired_state_quad_.position = quadrotor_common::geometryToEigen(
        state_estimate_quad_.pose.pose.position);
...
void DroneRacing::hardStopCallback(const std_msgs::EmptyConstPtr &msg) {
    ROS_INFO("Asked teacher [%d] times for help and executed [%d] times NN "
             "selection.",
             asked_teacher_n_times_, executed_nn_traj_);
...
void DroneRacing::networkCallback(
    const geometry_msgs::TwistStampedConstPtr &msg) {
    network_selection_ = Eigen::Vector3d(
        msg->twist.linear.x, msg->twist.linear.y, msg->twist.linear.z);
    data_saver_->setNetworkSelection(Eigen::Vector3d(
        msg->twist.linear.x, msg->twist.linear.y, msg->twist.linear.z));
}
...
bool DroneRacing::setGoalWasTriggered() const { return goal_selected_; }

bool DroneRacing::checkStateEstDesStateDivergence() {
    Eigen::Vector3d divergence_vec;
    divergence_vec =
        Eigen::Vector3d(desired_state_world_.position.x() -
                            state_estimate_world_.pose.pose.position.x,
                        desired_state_world_.position.y() -
                            state_estimate_world_.pose.pose.position.y,
                        desired_state_world_.position.z() -
    ...
    std_msgs::Float64 divergence_msg;
    divergence_msg.data = divergence_vec.norm();
    divergence_pub_.publish(divergence_msg);
...
Eigen::Vector3d DroneRacing::getEstimatedPos() {
    ...
    visualizer_->displayDebug(est_pos.x(), est_pos.y(), est_pos.z(), 4,
...
void DroneRacing::transformToQuadFrame() {
    ...
    desired_state_quad_.position =
        1.0 / scale_W_O *
        (q_W_O.inverse() * (desired_state_world_.position - t_W_O));
    desired_state_quad_.velocity =
        1.0 / scale_W_O * (q_W_O.inverse() * desired_state_world_.velocity);
    desired_state_quad_.acceleration =
        1.0 / scale_W_O * (q_W_O.inverse() * desired_state_world_.acceleration);
    desired_state_quad_.jerk =
        1.0 / scale_W_O * (q_W_O.inverse() * desired_state_world_.jerk);
    desired_state_quad_.snap =
        1.0 / scale_W_O * (q_W_O.inverse() * desired_state_world_.snap);
    desired_state_quad_.heading = quadrotor_common::wrapMinusPiToPi(
...
void DroneRacing::transformToWorldFrame() {
    ...
    desired_state_world_.position =
        1.0 / scale_W_O * ((q_W_O * desired_state_quad_.position) + t_W_O);
    desired_state_world_.velocity =
        1.0 / scale_W_O * (q_W_O * desired_state_quad_.velocity);
    desired_state_world_.acceleration =
        1.0 / scale_W_O * (q_W_O * desired_state_quad_.acceleration);
    desired_state_world_.jerk =
        1.0 / scale_W_O * (q_W_O * desired_state_quad_.jerk);
    desired_state_world_.snap =
        1.0 / scale_W_O * (q_W_O * desired_state_quad_.snap);
    desired_state_world_.heading = quadrotor_common::wrapMinusPiToPi(
        desired_state_quad_.heading +
        quadrotor_common::quaternionToEulerAnglesZYX(q_W_O).z());
}
...
void DroneRacing::transformStateEstimateToWorldFrame() {
    ...
    const Eigen::Vector3d t_W_O = T_W_S_.getPosition();
    double scale_W_O = 1.0;
    state_estimate_world_.pose.pose.position =
        quadrotor_common::vectorToPoint(quadrotor_common::eigenToGeometry(
            1.0 / scale_W_O *
            ((q_W_O * quadrotor_common::geometryToEigen(
                          state_estimate_quad_.pose.pose.position)) +
             t_W_O)));
    state_estimate_world_.twist.twist.linear =
        quadrotor_common::eigenToGeometry(
            1.0 / scale_W_O *
            (q_W_O * (quadrotor_common::geometryToEigen(
                         state_estimate_quad_.twist.twist.linear))));
    state_estimate_world_.pose.pose.orientation =
        quadrotor_common::eigenToGeometry(
            (q_W_O * quadrotor_common::geometryToEigen(
                         state_estimate_quad_.pose.pose.orientation)));
...
double DroneRacing::getDesiredVelocity() {
    quadrotor_common::TrajectoryPoint next_des_state_mb =
        global_trajectory_->getNextStateOnGlobalTraj(0.5);
    double max_velocity_global = global_trajectory_->getMaxVelocity();
    double desired_velocity_mb =
        next_des_state_mb.velocity.norm() / max_velocity_global;
    return desired_velocity_mb;
}

void DroneRacing::updateWaypointGoals() {
    visualizer_->displayDebug(
        desired_state_world_.position.x(), desired_state_world_.position.y(),
        desired_state_world_.position.z(), 3, visualizer_->Color::kRed);

    quadrotor_common::TrajectoryPoint des_state_proj =
        global_trajectory_->projectOnTrajectory(desired_state_world_);
    visualizer_->displayDebug(
        des_state_proj.position.x(), des_state_proj.position.y(),
        des_state_proj.position.z(), 1, visualizer_->Color::kBlue);

    quadrotor_common::TrajectoryPoint next_des_state_mb =
        global_trajectory_->getNextStateOnGlobalTraj(0.5);

    double desired_velocity_mb =
        std::min(next_des_state_mb.velocity.norm() /
                     global_trajectory_->getMaxVelocity() * max_velocity_,
                 max_velocity_);

    visualizer_->displayDebug(goal_positions_.at(curr_goal_index_).x(),
                              goal_positions_.at(curr_goal_index_).y(),
                              goal_positions_.at(curr_goal_index_).z(), 27,
                              visualizer_->Color::kBlack);

    Eigen::Vector3d dist_to_next_gate;
    dist_to_next_gate =
        desired_state_world_.position -
        Eigen::Vector3d(goal_positions_.at(curr_goal_index_).x(),
                        goal_positions_.at(curr_goal_index_).y(),
...
void DroneRacing::enableTestTimeCallback(const std_msgs::BoolConstPtr &msg) {
    test_time_ = msg->data;
    if (test_time_) {
        ROS_INFO("[%s] Enabled test time execution.",
                 ros::this_node::getName().c_str());
    }
}

double DroneRacing::constrain(const double &prev_value, const double &new_value,
...
void DroneRacing::executeTestTime() {
    ...
    if (mainloop_iter_ % plan_every_n_iter_ == 0) {
        ...
        bool success_nw = trajectory_manager_->computeTrajectory(
            this, desired_state_world_, state_estimate_world_,
            network_selection_);
        updateWaypointGoals();
...
void DroneRacing::loadParameters() {
    ...
    // General
    quadrotor_common::getParam<double>("trajectory_max_z", traj_max_z_, 2.5,
                                       pnh_);
    quadrotor_common::getParam<int>("plan_every_nth_iteration",
                                    plan_every_n_iter_, 1, pnh_);
    quadrotor_common::getParam<double>("max_velocity", max_velocity_, 1.0,
                                       pnh_);
    quadrotor_common::getParam<double>("min_velocity", min_velocity_, 1.0,
                                       pnh_);
    quadrotor_common::getParam<double>("max_divergence", max_divergence_, 0.2,
                                       pnh_);
    quadrotor_common::getParam<double>("max_error_to_global_traj_start",
                                       max_error_to_global_traj_start_, 1.0,
                                       pnh_);
    quadrotor_common::getParam<double>("max_error_to_global_traj_end",
                                       max_error_to_global_traj_end_, 1.0,
                                       pnh_);
    quadrotor_common::getParam<double>("d_replan", d_replan_, 1.0, pnh_);
    quadrotor_common::getParam<double>("camera_fov_yaw", camera_fov_deg_yaw,
                                       45.0, pnh_);
    quadrotor_common::getParam<double>("camera_fov_pitch", camera_fov_deg_pitch,
                                       45.0, pnh_);
    quadrotor_common::getParam<bool>("moving_gates", moving_gates_, false,
                                     pnh_);
    quadrotor_common::getParam<double>("gates_static_amplitude",
                                       gates_static_amp_, 0.0, pnh_);
    quadrotor_common::getParam<int>("max_failed_trials", max_failed_trials_, 5,
                                    pnh_);
    quadrotor_common::getParam<double>("wait_n_sec_till_record",
                                       sec_no_record_at_start_, 0.0, pnh_);
    quadrotor_common::getParam<bool>("record_data", record_data_, false, pnh_);
    quadrotor_common::getParam<bool>("perturb_actions", perturb_actions_, false,
                                     pnh_);
    quadrotor_common::getParam<std::string>("quad_frame", quad_frame_, "none",
                                            pnh_);
    quadrotor_common::getParam<int>("curr_goal_idx", curr_goal_index_, 0, pnh_);
